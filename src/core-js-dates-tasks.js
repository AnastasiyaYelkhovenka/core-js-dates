...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil(((currentDate - yearStart) / msPerDay + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  while (true) {
    ...
    if (friday13th.getDay() === 5 && friday13th > date) {
      return friday13th;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startDate = parseDateToObject(period.start);
  const endDate = parseDateToObject(period.end);
  const currentDate = new Date(startDate);

  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatDateToStr(currentDate));
